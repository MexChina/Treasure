...
func (tableModel Table) GetDataFromDatabase(queryParam map[string]string) ([]map[string]string, []map[string]template.HTML, types.PaginatorAttribute, string, string) {
	...
	columnsModel, _ := orm.GetConnectionByDriver(tableModel.ConnectionDriver).Query(showColumns)
	...
	res, _ := orm.GetConnectionByDriver(tableModel.ConnectionDriver).Query("select "+fields+" from "+tableModel.Info.Table+" where id > 0 order by "+queryParam["sortField"]+" "+
	...
	total, _ := orm.GetConnectionByDriver(tableModel.ConnectionDriver).Query("select count(*) from "+tableModel.Info.Table+" where id > ?", 0)
	var size int
	if tableModel.ConnectionDriver == "sqlite" {
...
func (tableModel Table) GetDataFromDatabaseWithId(prefix string, id string) ([]types.FormStruct, string, string) {
	...
	columnsModel, _ := orm.GetConnectionByDriver(tableModel.ConnectionDriver).Query("show columns in " + tableModel.Form.Table)
	columns := GetColumns(columnsModel, tableModel.ConnectionDriver)
	...
	res, _ := orm.GetConnectionByDriver(tableModel.ConnectionDriver).Query("select "+fields+" from "+tableModel.Form.Table+" where id = ?", id)
...
func (tableModel Table) UpdateDataFromDatabase(prefix string, dataList map[string][]string) {
	...
	orm.GetConnectionByDriver(tableModel.ConnectionDriver).Exec("update "+tableModel.Form.Table+" set "+fields+" where id = ?", valueList...)
}
...
func (tableModel Table) InsertDataFromDatabase(prefix string, dataList map[string][]string) {
	...
	// TODO: 过滤
	orm.GetConnectionByDriver(tableModel.ConnectionDriver).Exec("insert into "+tableModel.Form.Table+"("+fields+") values ("+queStr+")", valueList...)
}
...
func (tableModel Table) DeleteDataFromDatabase(prefix string, id string) {
	...
	for _, id := range idArr {
		orm.GetConnectionByDriver(tableModel.ConnectionDriver).Exec("delete from "+tableModel.Form.Table+" where id = ?", id)
	}
}
