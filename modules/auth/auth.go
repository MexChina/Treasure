...
func Check(password string, username string) (user User, ok bool) {
	...
	admin, _ := orm.GetConnection().Query("select * from goadmin_users where username = ?", username)
	...
	if len(admin) < 1 {
		...
		if ComparePassword(password, admin[0]["password"].(string)) {
			...
			roleModel, _ := orm.GetConnection().Query("select r.id, r.name, r.slug from goadmin_role_users "+
			...
			orm.GetConnection().Exec("update goadmin_users set password = ? where id = ?", newPwd, user.ID)
