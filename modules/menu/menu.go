...
func SetGlobalMenu(user auth.User) {
	...
	if user.IsSuperAdmin() {
		menus, _ = orm.GetConnection().Query("select * from menu where id > 0 order by `order` asc")
	} else {
		...
		menus, _ = orm.GetConnection().Query("select * from menu where id in ("+qmark[:len(qmark)-1]+") "+
...
func GetMenuItemById(id string) MenuItem {
	menu, _ := orm.GetConnection().Query("select * from menu where id = ?", id)
